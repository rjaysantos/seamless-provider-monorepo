...
class JdbEncryptionTest extends TestCase
{
    ...
    public function test_decrypt_validEncryption_expected()
    {
        ...
        $encryption = $this->makeEncryption();
        $response = $encryption->decrypt(credentials: $credentials, data: $data);

        $this->assertEquals(expected: $expected, actual: $response);
    }

    public function test_decrypt_mockLogger_logDecrypted()
    {
        $expected = (object) [
            'action' => 21,
            'ts' => 1609430400000,
        ...
        $encryption = $this->makeEncryption();
        $encryption->decrypt(credentials: $credentials, data: $data);
    }

    public function test_decrypt_stubLogger_expected()
    {
        $expected = (object) [
            'action' => 21,
            'ts' => 1609430400000,
        ...
        $stubLogger = $this->createMock(Logger::class);
        $this->app->instance(Logger::class, $stubLogger);
        $stubLogger->method('logDecrypted')
            ->with((array) $expected);
